...
colorChecker_customGray <- function(IDlist,
                         prepath = NULL,
                         extension = NULL,
  ...
  for(n in 1:length(IDlist)){
    ...
    if(colorCheckerType == 'X-Rite' & is.null(fixedModel)){
      if(is.null(colorCheckerXY)){
        print("Click on the outer corners of the X-Rite ColorChecker (1: ~white top, 2: ~black top, 3: ~black bottom, 4: ~white bottom)")
      ...
      xdiff <- (line2$x[2]-line2$x[1])/1
      ydiff <- (line2$y[2]-line2$y[1])/1

      xmin <- line2$x[1]
      ymin <- line2$y[1]

      xySubA <- list(x = c((xmin+xmin+xdiff)/2),
                     y = c((ymin+ymin+ydiff)/2))

      xySubDF_2A <- as.data.frame(xySubA)

      xySubBa <- list(x = c(xmin+xdiff-(xdiff/2)*prop),
                      y = c(ymin+ydiff-(ydiff/2)*prop))
      xySubBb <- list(x = c(xmin+(xdiff/2)*prop),
                      y = c(ymin+(ydiff/2)*prop))

      xySubDF_2Ba <- as.data.frame(xySubBa)
      xySubDF_2Bb <- as.data.frame(xySubBb)

      xdiff <- (line4$x[2]-line4$x[1])/1
      ydiff <- (line4$y[2]-line4$y[1])/1
      ...
      for(e in 1:nrow(xySubDF_1A)){
        ...
        xdiff <- (xyLine2$x-xyLine1$x)/1
        ydiff <- (xyLine2$y-xyLine1$y)/1
        ...
        xdiffa <- (xyLine3a$x-xyLine1a$x)/1
        ydiffa <- (xyLine3a$y-xyLine1a$y)/1

        xdiffb <- (xyLine3b$x-xyLine1b$x)/1
        ydiffb <- (xyLine3b$y-xyLine1b$y)/1
        ...
        xySubAa <- list(x = c(xmina+xdiffa-(xdiffa/2)*prop),
                        y = c(ymina+ydiffa-(ydiffa/2)*prop))
        xySubAb <- list(x = c(xmina+(xdiffa/2)*prop),
                        y = c(ymina+(ydiffa/2)*prop))

        xySubBa <- list(x = c(xminb+xdiffb-(xdiffb/2)*prop),
                        y = c(yminb+ydiffb-(ydiffb/2)*prop))
        xySubBb <- list(x = c(xminb+(xdiffb/2)*prop),
                        y = c(yminb+(ydiffb/2)*prop))
      ...
      for(e in 1:nrow(xyTot)){
        ...
        print(paste('Calculating observed RGB values for patch', e+18, sep = ' '))
        if(is.null(colorCheckerXY)){
      ...
      ColorCheckerRGB <- as.data.frame(rbind(l19,l20,l21,l22,l23,l24))
      ...
      # model estimation seems to be bad with gray scales
